#!/usr/bin/env python3

import sys

# Define a dictionary that maps color names to CMYK values
color_dict_cmyk = {
    "red": (0, 100, 100, 0),
    "green": (100, 0, 100, 0),
    "blue": (100, 100, 0, 0),
    "white": (0, 0, 0, 0),
    "black": (0, 0, 0, 100),
    # Add more color mappings as needed
}

def color_to_cmyk(color_name):
    # Convert the input color name to lowercase for case-insensitive matching
    color_name = color_name.lower()

    # Check if the color_name exists in the dictionary
    if color_name in color_dict_cmyk:
        return color_dict_cmyk[color_name]
    else:
        return None  # Return None for undefined colours

def main():
    # Check if a color name is provided as a command-line argument
    if len(sys.argv) != 2:
        print("Usage: ./color_to_cmyk.py <color>")
        sys.exit(1)

    color_name = sys.argv[1]
    cmyk = color_to_cmyk(color_name)

    if cmyk is not None:
        print(f"The CMYK values for {color_name} are {cmyk}")
    else:
        print(f"Color '{color_name}' is not defined in the dictionary.")

if __name__ == "__main__":
    main()

